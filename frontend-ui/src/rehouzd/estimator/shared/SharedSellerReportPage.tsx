...
import SellerReport from '../report/seller/SellerReport';
...
interface SharedReportData {
  ...
  estimateData: any;
  reportType?: string; // 'seller' or 'investor'
}
...
const SharedSellerReportPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSharedReport = async () => {
      ...
      try {
        ...
        // Check if this is a seller report
        if (data.data.reportType !== 'seller' && !data.data.estimateData?.displayedInvestorComps) {
          setError({
            code: 'INVALID_TOKEN',
            message: 'This link is for an investor report, not a seller report.',
          });
          setIsLoading(false);
          return;
        }
        ...
        console.error('Error fetching shared seller report:', error);
        
        const errorCode = error.message || 'SERVER_ERROR';
        const errorMessages: Record<string, string> = {
          NOT_FOUND: 'This shared seller report could not be found.',
          EXPIRED: 'This shared seller report has expired.',
          INACTIVE: 'This shared seller report is no longer active.',
          INVALID_TOKEN: 'The shared seller report link is invalid.',
          SERVER_ERROR: 'An error occurred while loading the seller report.',
        };
  ...
  // Success state - render the seller report
  if (!sharedReport) {
    return null;
  }
      ...
      <Box bg={cardBg} borderTop="1px" borderColor="gray.200" py={6} mt={8} className="no-print">
        <Container maxW="container.xl">
          <VStack spacing={4} textAlign="center">
            <Text fontSize="sm" color="gray.600">
              This seller report was generated by <strong>Rehouzd</strong> - Professional Real Estate Investment Analysis
            </Text>
            <Button
              colorScheme="brand"
              onClick={() => navigate('/')}
...
export default SharedSellerReportPage;
