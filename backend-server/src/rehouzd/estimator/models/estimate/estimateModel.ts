...
// Gets a specific estimate by address_id.
export const getEstimateByAddressId = async (addressId: number): Promise<Estimate | null> => {
  const queryText = `
  ...
  try {
    ...
    if (result.rows.length > 0) {
      console.log(`Estimate with address_id ${addressId} retrieved successfully!`);
      return result.rows[0] as Estimate;
    }
